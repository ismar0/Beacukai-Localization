...
frappe.ui.ThemeSwitcher = class ThemeSwitcher {
	...
	root.setAttribute("data-theme-mode", this.current_theme);
	root.setAttribute("data-theme", this.current_theme);

	frappe.show_alert(__("Theme Changed to " + this.current_theme), 3);

	frappe.xcall("frappe.core.doctype.user.user.switch_theme", {
		theme: this.current_theme.charAt(0).toUpperCase() + this.current_theme.slice(1),
	});
}
