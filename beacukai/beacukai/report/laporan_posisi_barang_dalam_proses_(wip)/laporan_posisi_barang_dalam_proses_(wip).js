...
frappe.query_reports["LAPORAN POSISI BARANG DALAM PROSES (WIP)"] = {
    filters: [
        {
            fieldname: "from_date",
            label: "From Date",
...
function getReportData() {
    ...
    frappe.call({
        method: "beacukai.beacukai.report.laporan_posisi_barang_dalam_proses_(wip).laporan_posisi_barang_dalam_proses_(wip).execute",
...
function loadTabulatorTable(data = []) {
    ...
    if (!tabulatorInstance) {
        tabulatorInstance = new Tabulator("#tabulator-table", {
            layout: "fitColumns",
            columns: [
				{ title: "Kode Barang", field: "item_code", widthGrow: 1.5, headerFilter: "input" },
				{ title: "Nama Barang", field: "item_name", widthGrow: 1.5, headerFilter: "input" },
				{ title: "Satuan Barang", field: "uom", widthGrow: 1.5, headerFilter: "input" },
				{ title: "Jumlah Barang", field: "qty", sorter: "number", widthGrow: 1.5,  headerFilter: "input"},
				{ title: "Keterangan", field: "remarks", sorter: "number", widthGrow: 1.5, headerFilter: "input"},
            ],
    ...
    if (!exportWrapper) {
        ...
        exportExcelBtn.onclick = () => tabulatorInstance.download("xlsx", "posisi_barang_wip.xlsx");
        ...
        exportPdfBtn.onclick = () => tabulatorInstance.download("pdf", "posisi_barang_wip.pdf", { orientation: "landscape", title: "LAPORAN POSISI BARANG DALAM PROSES (WIP)" });
